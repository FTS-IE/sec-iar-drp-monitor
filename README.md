...
- `processed/` stores normalized representatives, DRP occurrences, and DRP rollups.
- `reports/` stores the Markdown summary and DRP change CSV.
...
The workflow runs tests, downloads and processes the current SEC/IAPD IAR compilation feed, writes stable latest files needed for the next comparison, commits only lightweight latest outputs back to the repo, uploads the latest report files as a 30-day artifact, emails the summary when DRP changes are detected, and emails a failure alert when the workflow fails.
...
The output tracks DRP disclosure flags in the SEC/IAPD compilation feed. A flag is
a source-data signal for reporting follow-up, not an independent finding that
misconduct occurred.
